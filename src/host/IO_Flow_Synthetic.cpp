...
bool gc_started_flag_for_debug = true;
unsigned int Total_page_movements_for_debug = 0;
unsigned int Total_gc_executions_for_debug = 0;
unsigned long long int Total_RMW_SEC = 0; //READ/WRITE AMP in sector size
...
namespace Host_Components
{
	...
	Host_IO_Request* IO_Flow_Synthetic::Generate_next_request()
	{
		...
		if (STAT_generated_request_count == 1451664-10)
		{
			PRINT_MESSAGE("Trace-------------");
		}
		...
		if (STAT_generated_request_count == (total_requests_to_be_generated/ PORTION_PRECOND)) {
			...
			Simulator->sim_time_before_PRECOND = Simulator->Time();
		}
#endif
#if ANLZ_AFTER_GC
		if (GC_on_for_debug && gc_started_flag_for_debug) {
			STAT_generated_request_count_before_GC = STAT_generated_request_count;
			Simulator->sim_time_before_GC = Simulator->Time();
			gc_started_flag_for_debug = false;
		}
#endif
		//DATE22
		if (STAT_generated_request_count % 20000 == 0) {
			std::cout << "0-1. IOPS (W): " << ((double)STAT_generated_write_request_count / (Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF)) << ", rqs: " << STAT_generated_write_request_count << std::endl;
			std::cout << "0-2. IOPS (R): " << ((double)STAT_generated_read_request_count / (Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF))<<",rqs: "<<STAT_generated_read_request_count << std::endl;
			if (GC_on_for_debug) {
				std::cout << "1. IOPS (total) after GC: " << ((double)STAT_generated_request_count - (double)STAT_generated_request_count_before_GC) / (((double)(Simulator->Time()) - ((double)(Simulator->sim_time_before_GC))) / (double)SIM_TIME_TO_SECONDS_COEFF) << std::endl;
				//std::cout << "2. Total page moves(subpg): " << (Total_page_movements_for_debug) << ", Total GC: " << (Total_gc_executions_for_debug) << std::endl;
				//std::cout << "3. Total RMW count (KB): " << Total_RMW_SEC / 2 << std::endl;
			}
			std::cout  << std::endl;
		}
