	...
	*/
	...
	public:
		Data_Cache_Manager_Flash_Advanced(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
			...
			unsigned int stream_count, unsigned int sector_no_per_page, unsigned int back_pressure_buffer_max_depth, unsigned int flush_unit_count);
	...
	private:
		...
		std::list<NVM_Transaction*>* waiting_writeback_transactions;
